...
func _ready():
	# Position labels below the circles, centered horizontally
...
func _draw():
	var center = get_size() / 2
	# Animated expanding/contracting circle (semi-transparent blue)
	draw_circle(center, _radius, Color(0.3, 0.6, 1.0, 0.5))
	# Static circle showing minimum radius (solid blue)
...
func _input(event):
	if event.is_action_pressed("ui_accept"):
		if abs(_radius - min_radius) <= success_tolerance:
			...
			# Missed timing — optional feedback here
			pass

# This handles tween setting _radius and schedules redraw
